...
app.post("/bookings", (req, res) => {
  ...
  const sql = "INSERT INTO bookings (user_id, slot_id, vehicle_type_id, vehicle_number) VALUES (?, ?, ?, ?)";
...
app.get("/bookings", (req, res) => {
  const sql = "SELECT b.booking_id, u.full_name, p.slot_number, b.booking_status FROM bookings b JOIN users u ON b.user_id = u.user_id JOIN parking_slots p ON b.slot_id = p.slot_id";
...
app.post("/transactions", (req, res) => {
  ...
  const sql = "INSERT INTO transactions (booking_id, amount_paid, payment_method) VALUES (?, ?, ?)";
